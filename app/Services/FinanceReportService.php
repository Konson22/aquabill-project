...
class FinanceReportService
{
    /**
     * @return array{
     *     financeSummary: array<string, mixed>,
     *     monthlyCollectionSummary: list<array{month: string, collected: float}>,
     *     zoneRevenueComparison: list<array{zone: string, collected: float}>,
     *     financeFilters: array<string, int|string|null>,
     *     filterOptions: array<string, mixed>,
     *     overdueBills: list<array<string, mixed>>,
     *     overdueBillsMeta: array{total_count: int, total_outstanding: float}
     * }
     */
    public function buildForRevenuePage(Request $request): array
    {
        ...
        $billPaymentsQuery->whereHas('payments', function (Builder $pq) use ($filters): void {
            $pq->when($filters['from'], fn (Builder $q, string $from) => $q->whereDate('payment_date', '>=', $from))
                ...
                ->when($filters['station_id'], fn (Builder $q, int $stationId) => $q->where('station_id', $stationId));
        });
        ...
        [$overdueBillsList, $overdueBillsMeta] = $this->scopedOverdueBillsPayload($filters);
        ...
        return [
            'financeSummary' => [
                ...
                'overdue_bills' => $overdueBillsMeta['total_count'],
    ...
    public function exportCsv(Request $request): StreamedResponse
    {
        ...
        return response()->stream(function () use ($rows, $filters): void {
            ...
            fputcsv($out, ['Station ID', $filters['station_id'] ?? '']);
            fputcsv($out, []);
    ...
    private function validatedFilters(Request $request): array
    {
        ...
        return [
            'from' => $validated['pf_from'] ?? null,
            'to' => $validated['pf_to'] ?? null,
    ...
    private function applyServiceChargeFilters(Builder $query, array $filters): void
    {
        $query
            ...
            ->when($filters['station_id'], function (Builder $q, int $stationId): void {
                $q->whereHas('payments', fn (Builder $p) => $p->where('station_id', $stationId));
            });
    }

    /**
     * @param  array<string, int|string|null>  $filters
     * @return array{0: list<array<string, mixed>>, 1: array{total_count: int, total_outstanding: float}}
     */
    private function scopedOverdueBillsPayload(array $filters): array
    {
        $today = Carbon::today();
        $overdueBaseQuery = Bill::query()
            ->whereIn('status', ['pending', 'partial'])
            ->whereDate('due_date', '<', $today->toDateString());
        $this->applyBillScopeFilters($overdueBaseQuery, $filters);
        ...
        $list = (clone $overdueBaseQuery)
            ...
            ->map(fn (Bill $bill): array => [
                ...
                'reference' => $bill->bill_no ?? ('BILL-'.str_pad((string) $bill->id, 6, '0', STR_PAD_LEFT)),
                'status' => $bill->status,
