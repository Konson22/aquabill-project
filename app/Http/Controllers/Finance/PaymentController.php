...
class PaymentController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return $this->renderFinanceOrAdmin('payments/index', [
            'payments' => $payments,
    ...
    public function report(Request $request)
    {
        ...
        $billStatusRow = Bill::query()
            ->selectRaw("SUM(CASE WHEN status = 'pending' THEN 1 ELSE 0 END) as pending")
            ->selectRaw("SUM(CASE WHEN status = 'fully paid' THEN 1 ELSE 0 END) as fully_paid")
            ->selectRaw("SUM(CASE WHEN status = 'forwarded' THEN 1 ELSE 0 END) as forwarded")
            ->selectRaw("SUM(CASE WHEN status = 'partial paid' THEN 1 ELSE 0 END) as partial_paid")
        ...
        $billKpis = [
            ...
            'billStatusCounts' => [
                'pending' => (int) ($billStatusRow->pending ?? 0),
                'fully_paid' => (int) ($billStatusRow->fully_paid ?? 0),
                'forwarded' => (int) ($billStatusRow->forwarded ?? 0),
                'partial_paid' => (int) ($billStatusRow->partial_paid ?? 0),
                'cancelled' => (int) ($billStatusRow->cancelled ?? 0),
            ],
        ];
        ...
        // Tariff revenue (all time, for Tariff breakdown card)
        ...
        // Zone revenue (all time, for reports)
        ...
        return $this->renderFinanceOrAdmin('payments/report', [
    ...
    public function show($id)
    {
        ...
        return $this->renderFinanceOrAdmin('payments/show', [
            'payment' => $payment,
            'payable' => $payable,
    ...
    public function destroy($id)
    {
        if (Auth::user()->department !== 'admin') {
            abort(403, 'Only admin department users can delete payment records.');
        }
