...
export default function ValvesEdit({ valve, zones, pipes }) {
    const breadcrumbs = [
        { title: 'GIS', href: '/gis' },
        { title: 'Valves', href: '/gis/valves' },
        { title: valve.valve_code, href: route('gis.valves.show', valve.id) },
        { title: 'Edit', href: route('gis.valves.edit', valve.id) },
    ];

    const [Picker, setPicker] = useState(null);
    const { data, setData, put, processing, errors } = useForm({
        valve_code: valve.valve_code,
        zone_id: valve.zone_id != null ? String(valve.zone_id) : '',
        pipe_id: valve.pipe_id != null ? String(valve.pipe_id) : '',
        valve_type: valve.valve_type,
        latitude: valve.latitude,
        longitude: valve.longitude,
        status: valve.status,
        installation_date: valve.installation_date ? String(valve.installation_date).slice(0, 10) : '',
    ...
    const submit = (e) => {
        e.preventDefault();
        put(route('gis.valves.update', valve.id));
    };

    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={`Edit ${valve.valve_code}`} />
            <div className="mx-auto flex max-w-4xl flex-col gap-6 p-6">
                <div className="flex items-center gap-4">
                    <Button variant="outline" size="icon" asChild>
                        <Link href={route('gis.valves.show', valve.id)}>
                            <ArrowLeft className="h-4 w-4" />
                        </Link>
                    </Button>
                    <h1 className="text-2xl font-bold tracking-tight">Edit valve</h1>
                </div>
