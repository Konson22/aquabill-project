...
const breadcrumbs = [
    ...
    { title: 'Create', href: '/gis/water-points/create' },
];

export default function WaterPointsCreate({ zones, waterPointTypes }) {
    const [Picker, setPicker] = useState(null);
    const { data, setData, post, processing, errors } = useForm({
        code: '',
        name: '',
        water_point_type_id: waterPointTypes[0]?.id ? String(waterPointTypes[0].id) : '',
        zone_id: '',
        latitude: '',
        longitude: '',
        manager_name: '',
    ...
    const submit = (e) => {
        e.preventDefault();
        post(route('gis.water-points.store'));
    };

    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Create water point" />
            <div className="mx-auto flex max-w-4xl flex-col gap-6 p-6">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-2xl font-bold tracking-tight">New water point</h1>
                        <p className="text-sm text-muted-foreground">Click the map to set coordinates.</p>
                    </div>
                </div>
